...
class Scene3(CScene):
    ...
    def Enter(self):
        ...
        weapon = Boomerang(960,550,2,135)
        for i in range(3):
    ...
    def Update(self):
        for group in self.arrObj.values():
            for sprite in group:
                sprite.Update()
    def start_game(self):
        for wp in sceneMgr.GetCurrentScene().arrObj[OBJECT_TYPE.ENEMY]:
            if wp.active == True:
                return
            
        sceneMgr.ChangeScene(SCENE_TYPE.SCENE4.value)
        return
    def exit_game(self):
        print("게임 시작!")
        return